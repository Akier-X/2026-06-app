...
export default function CoachScreen() {
  ...
  const send = async () => {
    ...
    setSending(true);
    try {
      const reply = await sendToCoach([...chat, userMessage]);
      appendChat({
        id: newId(),
        role: 'assistant',
        text: reply,
      ...
      appendChat({
        id: newId(),
        role: 'assistant',
        text: 'ごめんなさい、いま通信がうまくいかないようです。少し時間をおいて試してみてください。',
        createdAt: new Date().toISOString(),
      });
    } finally {
      setSending(false);
        ...
        renderItem={({ item }) => {
          ...
          return (
            <View
              ...
              <Text style={{ color: mine ? '#fff' : c.text, fontSize: 15, lineHeight: 21 }}>
                {item.text}
              </Text>
            </View>
          );
